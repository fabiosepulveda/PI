...
data.crop$Age.crop = cons.date - data.crop$date.crop
data.eto$date <- as.Date(data.eto$date)
consultation = as.Date(cons.date)
data.crop$Age.crop = consultation - data.crop$date.crop
farms.crop.cons <- subset(data.crop, data.crop$date.crop <= consultation)
